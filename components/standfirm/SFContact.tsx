...
export default function SFContact() {
  ...
  const phone2 = c("phone2", sf.phones[1] ?? "");
  const whatsappNumber = c("whatsapp", sf.whatsapp);
  const whatsappDisplay = c("whatsapp", sf.whatsappDisplay);
  const hours = [
    { ...sf.hours[0], h: c("hoursWeekday", sf.hours[0].h) },
    { ...sf.hours[1], h: c("hoursSunday", sf.hours[1].h) },
  ];
    ...
    <section id="contact" className="relative bg-obsidian section-pad">
      ...
      <div className="mx-auto mt-12 grid max-w-6xl gap-6 lg:grid-cols-[1fr_1.1fr]">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          ...
          >
            <MessageCircle size={16} /> {ta ? "வாட்ஸ்அப்" : "WhatsApp"} {whatsappDisplay}
          </a>
        </div>
