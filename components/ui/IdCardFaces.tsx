 ...
 * is redrawn, cropped, or covered — only the live data (member
 * fields, the photo, the QR code, the emergency contact number) is
 * positioned on top, landing exactly on the blank lines already
 * printed on the template. The template's own "Signature" line and
 * its QR corner graphic are left completely alone; nothing is drawn
 * over them.
 *
...
/* QR corner, measured from the template: x[518,594] y[6,98]. The QR
   itself is always square, so it's sized to the smaller of that
   box's two dimensions and centred — no background box of its own is
   drawn behind it (the QR image is already fully opaque), so there
   is nothing to sit misaligned against the template's own artwork. */
const QR_BOX = { left: bx(518), top: by(6), width: bx(594 - 518), height: by(98) - by(6) };
const QR_SIZE = Math.round(Math.min(QR_BOX.width, QR_BOX.height) - 4);
  ...
  return (
    <div ref={cardRef} style={shell}>
      ...
      {/* QR — the image itself is fully opaque, so it's placed directly
          with no wrapping box of its own */}
      <div style={{ position: "absolute", left: QR_LEFT, top: QR_TOP }}>
        <QrCode value={data.verifyUrl} size={QR_SIZE} />
      </div>
