...
export default function Contact() {
  ...
  const phone1 = c("phone1", site.phones[0]);
  const phone2 = c("phone2", site.phones[1] ?? "");
  const landline = c("landline", site.landline);
  const address = c("address", lang === "ta" ? site.addressTa : site.address);
        ...
        <div className="contact-col space-y-6">
          ...
          <div className="glass gold-border rounded-2xl p-7 space-y-4 font-sans text-sm">
            <p className="flex items-start gap-3 text-ivory/90">
              <MapPin size={17} className="mt-0.5 shrink-0 text-gold" /> {address}
            </p>
            ...
            <div className="pt-2 flex flex-wrap gap-3 text-gold">
              {[phone1, phone2].filter(Boolean).map((p) => (
              ...
              <span className="rounded-full gold-border px-4 py-2 text-xs tracking-widest">{landline}</span>
            </div>
          </div>
        </div>
