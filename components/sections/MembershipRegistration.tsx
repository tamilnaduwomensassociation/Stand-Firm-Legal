...
function FieldInput({ f, value, onChange, lang, invalid, error }: {
  ...
  /* "Other" on any select: choosing it swaps the dropdown for a free-text
     box instead of forcing the closed list on a genuine edge case. The
     dropdown itself keeps showing "Other" selected even while the typed
     value (which isn't one of f.options) is what actually gets saved —
     that's the `otherMode` flag below, independent of the value being
     empty mid-type. */
  const hasOther = f.type === "select" && (f.options ?? []).includes("Other");
  const [otherMode, setOtherMode] = useState(
    () => hasOther && value !== "" && !(f.options ?? []).includes(value)
  );
        ...
        <>
          ...
          {otherMode && (
            <input
              type="text"
              autoFocus
              placeholder={lang === "ta" ? "இங்கே தட்டச்சு செய்யவும்" : "Please specify"}
              className={cn(inputCls, "[color-scheme:light] mt-2", ring)}
              value={value}
...
export default function MembershipRegistration({ embedded = false }: { embedded?: boolean }) {
  ...
  /* A category's extra fields (e.g. advocate's Bar Council Enrolment
     Number) can carry a `default` — see config/forms.config.ts. Seeded
     into `vals` the moment a category is chosen, so the field shows
     pre-filled instead of empty; `prev[f.id] ?? default` never
     overwrites something the applicant already typed, including on a
     re-render after switching categories and back. */
  useEffect(() => {
    if (!cat) return;
    const defaults = cat.extraFields.filter((f) => f.default);
  ...
  const steps = useMemo(() => {
    ...
    return [
      ...
      { en: "ID Card", ta: "அடையாள அட்டை", fields: [] as Field[] },
  ...
  const isIdCard = cat ? step === 6 : false;
  const isPayment = cat ? step === 7 : false;
    ...
    >
      ...
      <div className={cn("reg-panel mx-auto grid max-w-5xl gap-8 lg:grid-cols-[1fr_290px]", embedded ? "mt-0" : "mt-10")}>
        <div className="glass gold-border rounded-2xl p-8">
          ...
          {/* ID Card step — same builder as the standalone /id-card page,
              embedded here so a member fills it in as part of one visit
              instead of a separate errand later. No ₹49 gate: the
              applicant pays once, for the membership itself, on the very
              next step. */}
          {isIdCard && (
            <div className="-mx-4 sm:-mx-6">
              <IdCardSection
                embedded
                initialData={{
                  memberName: vals.name ?? "",
                  mobile: vals.phone ?? "",
                  address: vals.address ?? "",
                  blood: vals.blood ?? "",
                }}
                initialPhoto={files.photo?.dataUrl ?? null}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      /* Only the first character is forced to uppercase —
                         everything after it is kept exactly as typed, so a
                         name typed in caps stays in caps and one typed in
                         lowercase stays lowercase, matching the keyboard. */
                      const raw = e.target.value;
                      const v = raw.length ? raw[0].toUpperCase() + raw.slice(1) : raw;
                      setVals((p) => ({ ...p, signature: v }));
                    }}
