...
export function themeCss(t: ThemeTokens, opts: { isolate?: boolean } = {}): string {
  ...
  if (accent)  lines.push(`--gold: ${accent};`);

  /* `initial` resets a custom property to a guaranteed-invalid value,
     which is exactly what makes var(--x, fallback) fall through to
     its fallback again — the right reset for the two properties below
     that are always read with a fallback. */
  if (t.onAccent) lines.push(`--on-gold: ${t.onAccent};`);
  else if (opts.isolate) lines.push(`--on-gold: initial;`);
