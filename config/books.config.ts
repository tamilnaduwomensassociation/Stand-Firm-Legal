 ...
 *    edition-agnostic path. No per-title price is stocked here (see
 *    below), because a bare act's cost changes with its edition; the
 *    association calls back with the current one.
 *
 * 2. "Buy now" — a flat, edition-independent SERVICE FEE for sourcing
 *    and delivering the title, payable online: ₹250 for a verified
 *    TNWLA member, ₹500 otherwise (BOOK_PRICE_MEMBER /
 *    BOOK_PRICE_NON_MEMBER below). Membership is verified server-side
 *    against the same directory "Verify Your Membership" uses — see
 *    app/api/books-payment/order/route.ts — never taken on the
 *    buyer's say-so.
 *
 ...
 * which is also why no per-title retail price is stocked here; the
 * flat service fee above is charged instead of one.
 */

/** Flat, per-title purchase-service fee — see the note above. */
export const BOOK_PRICE_MEMBER = 250;
export const BOOK_PRICE_NON_MEMBER = 500;
